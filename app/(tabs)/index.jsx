...
export default function HomeScreen() {
    ...
    const dueSupplements = useMemo(() => supplements.filter((s) => Array.isArray(s.daysOfWeek) && s.daysOfWeek.includes(todayDay)), [supplements, todayDay]);
    const visibleSupplements = useMemo(() => {
        if (!searchQuery.trim())
            return dueSupplements;
    ...
    const supplementsByTime = visibleSupplements.reduce((acc, s) => {
        if (!acc[s.timeMinutes])
            acc[s.timeMinutes] = [];
    ...
    return (<Screen header={<HomeHeader searchSlot={<View style={styles.searchUtility}>
              <Ionicons name="search" size={16} color={colors.brand.primary} style={styles.searchInlineIcon}/>

              <TextInput value={searchQuery} onChangeText={setSearchQuery} placeholder="Search supplements…" placeholderTextColor={colors.text.muted} style={styles.searchInputUtility} clearButtonMode="while-editing"/>
            </View>}/>}>
      <View style={styles.content}>
        {isSearching ? (<View style={styles.searchResults}>
            {matches.map((m) => (<Pressable key={m.id} onPress={() => {
                    setSearchQuery("");
                    setMatches([]);
                    router.push({
          ...
          </View>) : visibleSupplements.length === 0 ? (<EmptyState />) : (Object.entries(supplementsByTime)
            .sort(([a], [b]) => Number(a) - Number(b))
            .map(([timeMinutes, items]) => (<View key={timeMinutes} style={styles.timeSection}>
                <Text style={styles.timeLabel}>{items[0].time}</Text>
                <View style={styles.cardGroup}>
                  {items.map((s) => (<SupplementCard key={s.id} name={s.name} subtitle={s.dose} route={s.route} iconBackgroundColor={ratingColorFor(s.catalogId)} taken={Boolean(takenTimes[s.id])} footer={takenTimes[s.id]
                    ? `Taken at ${takenTimes[s.id]}`
                    : undefined} onPress={() => toggleTaken(s.id)} onLongPress={() => router.push({
