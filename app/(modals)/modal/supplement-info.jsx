...
export default function SupplementInfoModal() {
    ...
    const hasRating = data?.evidence_score != null;
    const ratingStyle = getRatingStyle(rating);
    const RatingIcon = rating < 50 ? SadIcon : rating < 75 ? NeutralIcon : HappyIcon;
    ...
    return (<ScrollView contentContainerStyle={styles.container} style={{ flex: 1 }}>
      {/* Header */}
      <View style={styles.headerBand}>
        <View style={styles.headerRow}>
          <View style={styles.avatar}>
            <Text style={styles.avatarInitial}>
              {(fallbackName?.[0] ?? "S").toUpperCase()}
            </Text>
          </View>
        ...
        <View style={styles.verificationRow}>
          {loaded && (<View style={[
              ...
              <Text style={[
                ...
                {isVerified ? "Verified information" : "User submitted · not verified"}
              </Text>
            </View>)}
        </View>

        {hasRating ? (<>
            <LinearGradient colors={ratingStyle.gradient} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={[styles.scoreCard, { borderColor: ratingStyle.border }]}>
              <View style={styles.scoreCardRow}>
                ...
                <Pressable onPress={handleHeartPress} style={styles.scoreIcon} hitSlop={8}>
          ...
          </>) : (<View style={[styles.unratedCard, styles.scoreCard]}>
            <Text style={styles.unratedText}>
              Not yet rated. This entry hasn’t been verified.
            </Text>
          </View>)}

        <View style={styles.metricRow}>
          {benefits.map((b) => (<Metric key={b.id} label={b.label} Icon={BENEFIT_ICON_MAP[b.label] ?? MedalIcon} color={b.icon === "gold"
      ...
      <View style={styles.whiteCard}>
        ...
        <SectionRow label="Risks & interactions" value={data?.risks_and_interactions}/>
      </View>
      ...
      <View style={styles.evidenceCard}>
        ...
        {benefits.length === 0 ? (<Text style={styles.emptyEvidence}>No evidence listed yet.</Text>) : (benefits.map((b) => (<EvidenceRow key={b.id} label={b.label} Icon={BENEFIT_ICON_MAP[b.label] ?? MedalIcon} color={b.icon === "gold"
                ? SOLID_METAL_COLORS.gold
                : b.icon === "silver"
                    ? SOLID_METAL_COLORS.silver
                    : SOLID_METAL_COLORS.bronze}/>)))}
...
/* ---------------- Components ---------------- */
function Metric({ label, color, Icon, }) {
    const nudge = BENEFIT_ICON_NUDGE[label] ?? 0;
    return (<View style={styles.metricItem}>
      <View style={styles.metricCircleWrapper}>
        <View style={[styles.metricCircle, { backgroundColor: color }]}>
          <Icon width={24} height={24} style={nudge ? { transform: [{ translateX: nudge }] } : undefined}/>
        </View>
      </View>
...
function SectionRow({ label, value, }) {
    ...
    return (<Pressable onPress={() => setOpen((prev) => !prev)} style={styles.sectionRow}>
