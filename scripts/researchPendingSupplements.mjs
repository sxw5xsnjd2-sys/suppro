...
async function requestResearch({
  ...
  return requestStructuredResponse({
    ...
    instructions: [
      ...
      "Before returning manual_review for identity ambiguity, first attempt to choose the most precise safe canonical active ingredient. For example: 'L-Acetyl-Cysteine' should become 'N-Acetylcysteine' if supported; 'Bamboo' should become a specific bamboo silica or bamboo extract only if the source identity supports it; 'Watermelon Extract' may be created as 'Watermelon Extract' only if the citations support that preparation as a supplement ingredient. Use create_precise for this. Return manual_review only when even a precise canonical_name would still be unsafe, too broad, product-like, or duplicate-prone.",
