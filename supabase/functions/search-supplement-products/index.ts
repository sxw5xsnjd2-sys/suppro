...
import {
  runFederatedProductSearch,
  validateFederatedSearchRequest,
} from "../_shared/federated-product-search-policy.js";
import {
  createFederatedDiagnosticLogger,
  createFederatedProductProviders,
  createFederatedSearchCache,
...
Deno.serve(async (req) => {
  ...
  const entitlement = await assertActiveRevenueCatEntitlement({
  ...
  const quota = await enforceEdgeFunctionQuota({
    adminSupabase,
    policyKey: "search-supplement-products",
    userId: authenticatedUser.user.id,
  });
  if (quota.ok === false) {
  ...
  const validation = validateFederatedSearchRequest(await req.text());
  if (!validation.ok) return jsonResponse(validation.body, validation.status);

  try {
    const providers = createFederatedProductProviders({ adminSupabase, eanSearchToken });
    const cache = createFederatedSearchCache(adminSupabase, logDiagnostic);
    return jsonResponse(await runFederatedProductSearch({
      ...validation.value,
      providers,
      cache,
      timeouts: {
    ...
    logDiagnostic("search", error);
    return jsonResponse({
      error: "Product search failed.",
      code: "search_failed",
      requestId: validation.value.requestId,
      normalizedQuery: validation.value.normalizedQuery,
    }, 500);
  }
});
