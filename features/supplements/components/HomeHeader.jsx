...
const formatDay = (date) => new Date(date).toLocaleDateString("en-GB", { weekday: "short" });
const formatMonthYear = (date) => new Date(date).toLocaleDateString("en-GB", {
    month: "long",
    year: "numeric",
});
const formatFullDate = (date) => new Date(date).toLocaleDateString("en-GB", {
    day: "2-digit",
    month: "long",
});
const formatFullDateWithYear = (date) => new Date(date).toLocaleDateString("en-GB", {
    day: "2-digit",
    month: "long",
    year: "numeric",
});
...
export function HomeHeader({ searchSlot }) {
    ...
    return (<LinearGradient colors={gradients.header} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={[styles.container, { paddingTop: insets.top + spacing.md }]}>
      {/* Top row */}
      <View style={styles.topRow}>
        <Image source={require("@/assets/icons/Supprologo.png")} style={styles.logo}/>
      ...
      <ScrollView ref={scrollRef} horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.weekRow}>
        {dates.map((date, index) => {
            const active = date === selectedDate;
            return (<Pressable key={`${date}-${index}`} onPress={() => setSelectedDate(date)} style={[styles.dayItem, active && styles.dayItemActive]}>
      ...
      {searchSlot ? (<View style={styles.searchShell}>
