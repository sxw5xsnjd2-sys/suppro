...
export function HealthEntryModal({ visible, metric, onClose }) {
    ...
    if (!metric)
        return null;
    const label = METRIC_LABEL[metric] ?? metric ?? "Metric";
    const clampToScale = (n) => {
        ...
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ width: "100%" }}>
          <View style={styles.card}>
            ...
            <View style={styles.field}>
              ...
              <Slider minimumValue={1} maximumValue={10} step={1} value={clampToScale(value)} onValueChange={(v) => setValue(clampToScale(v))} minimumTrackTintColor={colors.brand.primary} maximumTrackTintColor={colors.border.subtle} thumbTintColor={colors.brand.primary}/>
            </View>

            <View style={styles.field}>
              <Text style={styles.label}>Note (optional)</Text>
              <TextInput value={note} onChangeText={setNote} placeholder="Short context…" placeholderTextColor={colors.text.muted} style={[styles.input, styles.textarea]} multiline/>
            </View>
