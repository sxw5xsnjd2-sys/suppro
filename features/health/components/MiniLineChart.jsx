...
export function MiniLineChart({ data, height = 120, min = 1, max = 10, }) {
    ...
    const normalizeDatum = (datum) => {
        if (typeof datum === "number") {
            return { value: datum, hasNote: false };
        }
    ...
    // ─────────────────────────────────────────────
    if (data.length === 1) {
        const datum = normalizeDatum(data[0]);
        const cx = width / 2;
        const cy = valueToY(datum.value);
        return (<View onLayout={onLayout} style={{ height }}>
    ...
    // 2+ points → line
    // ─────────────────────────────────────────────
    const points = data.map((rawDatum, index) => {
        const datum = normalizeDatum(rawDatum);
        const t = index / (data.length - 1); // 0..1
        const x = x0 + t * (x1 - x0);
        const y = valueToY(datum.value);
        return { x, y, hasNote: datum.hasNote };
    });
    const d = points
